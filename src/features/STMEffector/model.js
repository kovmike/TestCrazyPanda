import { createStore, createEffect, createEvent, forward, restore, sample, combine } from "effector";
...
const getBookFx = createEffect(async () => {
  return fetch("https://next.json-generator.com/api/json/get/NkcCg0Tf9").then((response) => response.json());
});
...
$filteredBook
  .on($sortAge, (book, direction) => [...book].sort((a, b) => direction * (b.age - a.age)))
  .on($sortBalance, (book, direction) => [...book].sort((a, b) => direction * (parseBalance(b.balance) - parseBalance(a.balance))));

$sortAge.on(sortAge, (direction, _) => (direction === 0 ? 1 : -1 * direction));
$sortBalance.on(sortBalance, (direction, _) => (direction === 0 ? 1 : -1 * direction));
...
sample({
  ...
  fn: (book, name) =>
    book.filter(
      (person) =>
        person.name.first.toLowerCase().includes(name.toLowerCase()) || person.name.last.toLowerCase().includes(name.toLowerCase())
    ),
  target: $filteredBook,
});
...
export const $pagesCount = combine($filteredBook, $rowOnPage, (book, rows) => Math.ceil(book.length / rows));
export const $currentPage = createStore(1).reset([sortBalance, search, setRowsCount, sortAge]);
export const $pageContent = combine($filteredBook, $currentPage, $rowOnPage, (book, page, rows) =>book.slice(page * rows - rows, page * rows));
